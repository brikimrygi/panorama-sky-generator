...
def project_face(img_array, face, face_size, selected_mode, no_pinch=False, v_scale_val=1.0):
    ...
    if selected_mode == "standard":
        ...
        v_ratio = z / d_safe
        px = ((theta + np.pi) / (2 * np.pi)) * (w_pano - 1)
        py = (0.5 - (v_ratio * v_scale_val / 2.0)) * (h_pano - 1)
...
st.set_page_config(page_title="Skybox Generator", page_icon=None, layout="centered")

st.title("Panorama to Skybox Generator")
...
if uploaded_file is not None:
    ...
    # Dynamic vertical stretch adjustment
    auto_v_scale = st.sidebar.checkbox("Auto-calculate vertical stretch", value=True, help="Automatically scales vertical height based on aspect ratio (recommended)")
    if auto_v_scale:
        v_scale_val = aspect / 2.0
    else:
        v_scale_val = st.sidebar.slider("Manual Vertical Stretch", 0.2, 3.0, float(aspect / 2.0), 0.05, help="Decrease value for ultra-wide images (like 21:9) to flatten/squish stretching")

    # Color & Noise Filters
    st.sidebar.subheader("Color & Noise Filters")
    color_filter = st.sidebar.selectbox(
        "Color Filter", 
        ["None", "Grayscale", "Greenscale (Night Vision)", "Black & White (Binary)"],
        help="Apply custom monochrome and color filters to the output textures"
    )
    grain_intensity = st.sidebar.slider(
        "Film Grain (Noise)", 
        0, 30, 0, 1, 
        help="Add randomized noise grain to hide sky color banding artifacts. Set to 0 to disable."
    )

    # Blur Settings
    st.sidebar.subheader("Blur Settings")
    faces_to_blur = st.sidebar.multiselect(
        "Select faces to blur", 
        ["top", "bottom", "front", "back", "left", "right"],
        help="Select which skybox faces you want to apply Gaussian blur to"
    )
    blur_radius = st.sidebar.slider(
        "Blur Radius (px)", 
        0.0, 20.0, 0.0, 0.5, 
        help="Set to 0.0 to disable blur. Higher values increase blur intensity."
    )
    ...
    if st.button("Start Conversion", type="primary"):
        with st.spinner("Processing image... Please wait."):
            ...
            for face in faces:
                ...
                # Apply classic filters (always enabled unless specific nf_ flag is checked)
                ...
                elif color_filter == "Greenscale (Night Vision)":
                    gray = face_image.convert("L")
                    zeros = Image.new("L", face_image.size, 0)
                    face_image = Image.merge("RGB", (zeros, gray, zeros))
                elif color_filter == "Black & White (Binary)":
                    face_image = face_image.convert("L").point(lambda x: 0 if x < 128 else 255).convert("RGB")

                # Apply Gaussian Blur if selected
                if blur_radius > 0.0 and face in faces_to_blur:
                    face_image = face_image.filter(ImageFilter.GaussianBlur(radius=blur_radius))

                # Apply Film Grain (Noise) if selected (applied after blur to keep noise sharp)
            ...
            # --- Previews Section ---
            st.markdown("---")
            st.subheader("Generated Textures Preview")
