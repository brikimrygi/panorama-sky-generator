...
# Bezpieczne pobranie metod obracania i odbijania dla biblioteki Pillow
...
st.write("Wczytaj płaskie lub sferyczne zdjęcie panoramiczne i wygeneruj tekstury do gier (np. Minecraft).")

uploaded_file = st.file_uploader("Wybierz plik panoramy (JPG / PNG)", type=["jpg", "jpeg", "png"])
...
if uploaded_file is not None:
    ...
    st.image(img, caption=f"Wczytana panorama: {uploaded_file.name} ({w_pano}x{h_pano}px, proporcje {aspect:.2f}:1)", use_container_width=True)
    
    st.sidebar.header("Ustawienia generatora")
    
    # Wybór rozmiaru ścian
    face_size = st.sidebar.selectbox("Rozmiar pojedynczej ściany (px)", [512, 1024, 2048, 4096], index=1)
    
    # Wybór trybu rzutowania
    mode_option = st.sidebar.selectbox(
        "Tryb rzutowania horyzontu", 
        ["Automatyczny (wykrywanie)", "Spherical (płaski horyzont)", "Standard (pełna sfera 360)"]
    )
    
    selected_mode = "auto"
    if mode_option == "Standard (pełna sfera 360)":
        selected_mode = "standard"
    elif mode_option == "Spherical (płaski horyzont)":
        selected_mode = "spherical"
    else:
        selected_mode = "standard" if (1.95 <= aspect <= 2.05) else "spherical"
        
    st.sidebar.subheader("Filtry i optymalizacje")
    no_pinch = st.sidebar.checkbox("Usuń efekt wirówki (No Pinch)", value=True, help="Wygładza bieguny (top/bottom)")
    
    # Filtry No-Filter
    st.sidebar.markdown("**Wyłączenie filtrów dla konkretnych ścian:**")
    nf_top = st.sidebar.checkbox("Brak filtra dla ściany TOP", value=False)
    nf_bottom = st.sidebar.checkbox("Brak filtra dla ściany BOTTOM", value=False)
    nf_front = st.sidebar.checkbox("Brak filtra dla ściany FRONT", value=False)
    nf_back = st.sidebar.checkbox("Brak filtra dla ściany BACK", value=False)
    nf_left = st.sidebar.checkbox("Brak filtra dla ściany LEFT", value=False)
    nf_right = st.sidebar.checkbox("Brak filtra dla ściany RIGHT", value=False)
    
    # Wybór sposobu zapisu
    output_option = st.sidebar.radio("Co chcesz wygenerować?", ["Wszystko (pojedyncze + siatki)", "Tylko starfield02.png (Siatka 3x2)", "Tylko pojedyncze ściany sky_*.png"])

    raw_mode = (output_option == "Tylko pojedyncze ściany sky_*.png")
    final_mode = (output_option == "Tylko starfield02.png (Siatka 3x2)")

    if st.button("Rozpocznij konwersję", type="primary"):
        with st.spinner("Przetwarzanie obrazu... Proszę czekać."):
            ...
            # Przygotowanie ZIP-a w pamięci RAM
            zip_buffer = io.BytesIO()
            
            with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                # Zapis pojedynczych plików do ZIP
                if not final_mode:
                    for face in faces:
                        # Kopia bottom z obrotem 90 CW dla pojedynczego pliku
                ...
                if not raw_mode:
                    # Budowanie pionowego paska 1x3 w pamięci
                    ...
                    # Generowanie pomocniczej siatki 3x2 z oznaczeniami
                    if not final_mode:
            ...
            # Udostępnienie przycisku pobierania ZIP
            st.download_button(
                label="📁 Pobierz paczkę ZIP z teksturami",
