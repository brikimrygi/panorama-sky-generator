...
python psg.py <nazwa_pliku> [rozmiar_sciany] [-sp|-st] [flagi_nf] [-raw|-final] [-np] [-vs skala] [-grain szum] [filtr_koloru]
...
Flagi optymalizacji i filtrów dodatkowych:
  ...
  -grain <int>    - Dodaje efekt automatycznego szumu/ziarna o zadanej sile (np. -grain 8).
                    Pomaga ukryć paski na gradientach nieba (color banding).
  -grayscale      - Nakłada filtr czarno-biały (odcienie szarości).
  -greenscale     - Nakłada filtr zielony (monochromatyczny efekt noktowizora).
  -bw             - Nakłada twardy próg czarno-biały (tylko 100% czerń i 100% biel).
...
- python psg.py panorama.jpg 2048 -sp -grain 6  (Dodaje ziarno do płaskiej panoramy)
- python psg.py panorama.jpg -greenscale -f     (Tryb noktowizora, tylko starfield02.png)
...
python psg.py <panorama_filename> [face_size] [-sp|-st] [nf_flags] [-raw|-final] [-np] [-vs scale] [-grain noise] [color_filter]
...
Optimization & Extra Filter Flags:
  ...
  -grain <int>    - Adds randomized Gaussian film grain noise of specific intensity (e.g., -grain 8).
                    Helps hide sky color banding artifacts.
  -grayscale      - Applies a black & white grayscale filter.
  -greenscale     - Applies a green monochromatic night-vision style filter.
  -bw             - Applies a hard black & white binary threshold filter (no grays).
...
- python psg.py panorama.jpg 2048 -sp -grain 8 (Adds noise grain to prevent sky banding, 2048)
- python psg.py panorama.jpg -greenscale -f    (Night-vision, starfield02.png only, 2048)
...
def generate_skybox(panorama_path, face_size=1024, mode="auto", nf_top=False, nf_bottom=False, nf_front=False, nf_back=False, nf_left=False, nf_right=False, no_pinch=False, v_scale_val=None, grain_intensity=0, color_filter=None, raw_mode=False, final_mode=False):
    ...
    try:
        ...
        for face in faces:
            ...
            # Warunkowe nakładanie filtrów geometrycznych
            ...
            # --- Nakładanie filtrów koloru (Greyscale, Greenscale, Binary) ---
            if color_filter == "grayscale":
                face_image = face_image.convert("L").convert("RGB")
            elif color_filter == "greenscale":
                gray = face_image.convert("L")
                zeros = Image.new("L", face_image.size, 0)
                face_image = Image.merge("RGB", (zeros, gray, zeros))
            elif color_filter == "binary":
                face_image = face_image.convert("L").point(lambda x: 0 if x < 128 else 255).convert("RGB")

            # --- Nakładanie automatycznego ziarna / szumu ---
            if grain_intensity > 0:
                arr = np.array(face_image).astype(np.int16)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Użycie: psg <nazwa_pliku_lub_sciezka.png> [rozmiar_sciany] [-sp|-st] [-nf] [-nft] [-nfb] [-nff] [-nfbk] [-nfl] [-nfr] [-np] [-vs skala] [-grain szum] [-grayscale|-greenscale|-bw] [-raw|-final] lub psg -help")
        sys.exit(1)
    ...
    while i < len(sys.argv):
        ...
        if arg_lower in ["-sp", "--sp", "-spherical", "--spherical"]:
            ...
            no_pinch = True
        elif arg_lower in ["-grayscale", "--grayscale"]:
            color_filter = "grayscale"
        elif arg_lower in ["-greenscale", "--greenscale"]:
            color_filter = "greenscale"
        elif arg_lower in ["-bw", "--bw", "-binary", "--binary"]:
            color_filter = "binary"
        elif arg_lower in ["-grain", "--grain"]:
            if i + 1 < len(sys.argv):
                try:
                    grain_intensity = int(sys.argv[i+1])
                    i += 1
                except ValueError:
                    print(f"Błąd: Wartość po fladze {sys.argv[i]} musi być liczbą całkowitą.")
                    sys.exit(1)
            else:
            ...
            try:
                ...
                print(f"Błąd: Niepoprawny argument '{sys.argv[i]}'. Dozwolone flagi: -sp, -st, -nf, -np, -vs, -grain, -grayscale, -greenscale, -bw, -raw, -final lub rozmiar.")
                sys.exit(1)
        i += 1
                
    generate_skybox(input_path, size, mode, nf_top, nf_bottom, nf_front, nf_back, nf_left, nf_right, no_pinch, v_scale_val, grain_intensity, color_filter, raw_mode, final_mode)